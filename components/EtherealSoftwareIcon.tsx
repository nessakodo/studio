...
const EtherealSoftwareIcon: React.FC<EtherealIconProps> = ({
  ...
  return (
    ...
    >
      {/* Abstract Code/Network Node Symbol - Enhanced default visibility */}
      <motion.path
         ...
         animate={{ pathLength: isActive ? 1 : 0.5, opacity: isActive ? 1 : 0.7 }}
         transition={{ duration: 0.8, delay: isActive ? 0.2 : 0, ease: "easeInOut" }}
      />
       <motion.circle cx="50" cy="50" r="10"
        initial={{ scale: 0.5, opacity: 0.8 }}
        animate={{ scale: isActive ? 1 : 0.5, opacity: isActive ? 1 : 0.8 }}
        transition={{ duration: 0.6, delay: isActive ? 0.4 : 0, ease: "easeOut" }}
       />
    </motion.svg>
  );
};
